...
### 2.6. DRY (Don't Repeat Yourself) & AHA (Avoid Hasty Abstractions)
...
## 3. Estándares de UI y Estilos (CSS Modules)
...
## 4. Tipado Estricto y Validación de Esquemas
...
### 5.1. Pruebas Unitarias y de Integración (Vitest + React Testing Library)
...
- **Comando de ejecución:** `npm run test` / `npm run test:watch`.

### 5.2. Pruebas End-to-End (E2E con Playwright)
...
## 6. Convenciones de Colaboración Humano - AI
...
4. **Verificación Continua:** Tras cada cambio significativo, ejecutar `npm run lint`, `npm run typecheck` y `npm run test` antes de dar la tarea por concluida.
